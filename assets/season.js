    ...
    renderWeeklyHighScoreAndEliminator(Number(weekSelect.value));
        ...
        async function populateWeekSelects() {
            ...
            var playedWeeks =
                ...
                state.sleeperPlayedWeeks : [state.currentWeek || 1];
...
if (!matchups) {
    ...
    async function renderTeamSchedule() {
        ...
        function renderTotwAndBotw() {
            ...
            function rowHtml(slotLabel, entry) {
                if (!entry || !entry.playerName) {
                    return (
                        '<div class="totw-card">' +
                        '<div class="totw-card-top"><span class="totw-name">No data</span><span class="totw-pos-badge">' + escapeHtml(slotLabel) + '</span></div>' +
                        '</div>'
                    );
                }
                return (
                    '<div class="totw-card">' +
                    '<div class="totw-card-top">' +
                    '<span class="totw-name">' + escapeHtml(entry.playerName) + '</span>' +
                    '<span class="totw-pos-badge">' + escapeHtml(slotLabel) + '</span>' +
                    '</div>' +
                    '<div class="totw-card-bottom">' +
                    '<span class="totw-owner">' + escapeHtml(entry.ownerName) + '</span>' +
            ...
            totwList.innerHTML = totw.map(function(entry) {
                if (entry.playerName) totwTotal += entry.points;
                return rowHtml(entry.slot, entry);
            }).join("") + (
                '<div class="totw-card totw-total-row">' +
                '<div class="totw-card-top"><span class="totw-name">Total</span></div>' +
                '<div class="totw-card-bottom"><span></span><span class="totw-points">' + totwTotal.toFixed(2) + '</span></div>' +
                '</div>'
            );
            ...
            botwList.innerHTML = botw.map(function(entry) {
                ...
                return rowHtml(entry.position, entry);
            }).join("") + (
                '<div class="totw-card totw-total-row">' +
                '<div class="totw-card-top"><span class="totw-name">Total</span></div>' +
                '<div class="totw-card-bottom"><span></span><span class="totw-points">' + botwTotal.toFixed(2) + '</span></div>' +
                '</div>'
            );
        }
    ...
    async function renderTransactions() {
        ...
        console.log(JSON.stringify(txns.filter(function(t) {
    ...
    function faabCardsHtml(rows) {
        ...
        return (
            ...
            .map(function(r) {
                ...
                var totalBidVolume = r.bids.reduce(function(sum, b) {
                    return sum + b.amount;
                }, 0);
    ...
    async function populateSeasonRecordsMemberSelector() {
        ...
        if (previousValue && owners.some(function(o) {
                return o.key === previousValue;
            })) {
    ...
    var RECORD_DEFS = [{
        ...
        {
            key: "leastPoints",
            title: "Least Points in a Week",
            showOpponent: true,
            mode: "score"
        },
        {
            key: "largestMarginVictory",
            title: "Largest Margin of Victory",
            showOpponent: true,
            mode: "margin"
        },
        {
            key: "smallestMarginVictory",
            title: "Smallest Margin of Victory",
            showOpponent: true,
            mode: "margin"
        },
        {
            key: "largestMarginDefeat",
            title: "Largest Margin of Defeat",
            showOpponent: true,
            mode: "margin"
        },
        {
            key: "smallestMarginDefeat",
            title: "Smallest Margin of Defeat",
            showOpponent: true,
            mode: "margin"
        },
    ];
