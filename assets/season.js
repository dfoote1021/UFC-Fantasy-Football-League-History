(function () {
  ...
  function isEspnYear(season) {
    return window.EspnLoader && window.EspnLoader.ESPN_SEASONS.indexOf(Number(season)) !== -1;
  }
  ...
  async function loadSleeperSeason(season) {
    ...
    try {
      ...
      state.playoffStartWeek = (league.settings && league.settings.playoff_week_start) || null;
      var isComplete = league.status === "complete";
      setHidden("final-badge", !isComplete);
      state.finalStandingsInfo = SleeperAPI.buildFinalStandings(state.rosterMap, state.winnersBracket);
      ...
      state.sleeperRunningRecordsByWeek = SleeperAPI.buildAllRunningRecords(state.allWeeksMatchups, state.playoffStartWeek);
  ...
  async function loadEspnSeason(season) {
    ...
    if (!window.EspnLoader) {
      showFatalError("espn-loader.js did not load. Check that assets/espn-loader.js exists in the repo and that index.html references it.");
      return;
    }
    try {
  ...
  function renderTeamsEspn() {
    ...
    standings.forEach(function (team) {
      ...
      card.innerHTML =
        ...
        "</strong></div><p>" +
        escapeHtml(team.displayName) +
        '</p><div class="team-stats-row"><span>Record</span><span>' +
        team.wins +
        "-" +
        team.losses +
        "-" +
        team.ties +
        '</span></div><div class="team-stats-row"><span>Points For</span><span>' +
        team.fpts.toFixed(2) +
        '</span></div><div class="team-stats-row"><span>Points Against</span><span>' +
  ...
  function renderTeamScheduleEspn() {
    ...
    schedule.forEach(function (game) {
      ...
      tr.innerHTML =
        ...
        game.result +
        "</td><td>" +
        game.myPoints.toFixed(2) +
        "</td><td>" +
        (game.opponentPoints !== null ? game.opponentPoints.toFixed(2) : "-") +
        "</td><td>" +
        escapeHtml(game.recordAfter) +
        "</td>";
  ...
  async function ensureAllWeeksMatchups() {
    ...
    state.allWeeksMatchups = await SleeperAPI.getAllWeeksMatchups(state.leagueId, SleeperAPI.MAX_SLEEPER_WEEK);
  ...
  function renderChampionBanner() {
    ...
    if (info && info.champion) {
      banner.hidden = false;
      var runnerUpText = info.runnerUp ? " — defeated " + escapeHtml(info.runnerUp.teamName) + " in the championship" : "";
      textEl.textContent =
        state.season +
        " Champion: " +
        info.champion.teamName +
        (info.champion.displayName && info.champion.displayName !== info.champion.teamName ? " (" + info.champion.displayName + ")" : "") +
        runnerUpText;
    } else {
      banner.hidden = true;
    }
  ...
  function renderStandings() {
    ...
    var info = state.finalStandingsInfo;
    var isComplete = state.dataSource === "espn" || (state.league && state.league.status === "complete");
    ...
    if (!tbody) return;
    tbody.innerHTML = "";
    var sortFn = state.dataSource === "espn" ? window.EspnLoader.sortStandings : SleeperAPI.sortStandings;
  ...
  function renderDivisionStandings() {
    ...
    var divisions = SleeperAPI.buildDivisionStandings(state.rosterMap, state.league, state.finalStandingsInfo);
  ...
  function renderBracket(containerId, bracketData) {
    ...
    if (state.dataSource === "espn") {
      ...
      rounds = SleeperAPI.buildBracketView(bracketData, state.rosterMap, state.seedMap, state.allWeeksMatchups, state.playoffStartWeek);
    }
  ...
  function rosterListHtml(teamSide) {
    ...
    var items = roster
      .map(function (p) {
        ...
        return '<li class="' + cls + '"><span>' + escapeHtml(p.name) + " (" + escapeHtml(p.position) + ")</span><span>" + pts + "</span></li>";
      })
      .join("");
    return "<ul>" + items + "</ul>";
  }
  ...
  async function populateWeekSelects() {
    ...
    var playedWeeks = state.sleeperPlayedWeeks && state.sleeperPlayedWeeks.length ? state.sleeperPlayedWeeks : [state.currentWeek || 1];
  ...
  async function renderMatchups() {
    ...
    var recordsThisWeek = (state.sleeperRunningRecordsByWeek && state.sleeperRunningRecordsByWeek[week]) || {};
  ...
  async function renderTeamSchedule() {
    ...
    var schedule = SleeperAPI.buildTeamSchedule(state.allWeeksMatchups, rosterId, state.rosterMap, state.sleeperRunningRecordsByWeek, state.playoffStartWeek);
    if (!schedule || schedule.length === 0) {
      tbody.innerHTML = '<tr><td colspan="6">No schedule data available.</td></tr>';
      return;
    }
  ...
  async function renderTeams() {
    ...
    standings.forEach(function (team) {
      var card = document.createElement("div");
      card.className = "team-card";
      var avatarHtml = team.avatar ? '<img src="' + team.avatar + '" alt="' + escapeHtml(team.teamName) + '" />' : "";
      ...
      card.innerHTML =
        ...
        '</span></div><div class="team-stats-row"><span>Points Against</span><span>' +
        team.fptsAgainst.toFixed(2) +
        '</span></div><div class="team-stats-row"><span>Transactions</span><span>' +
        txnCount +
        '</span></div><div class="team-stats-row"><span>Roster Size</span><span>' +
        team.players.length +
        "</span></div>";
      grid.appendChild(card);
    });
  ...
  async function renderWeeklyRoster() {
    ...
    try {
      ...
      var matchupsForWeek = state.allWeeksMatchups ? state.allWeeksMatchups[week] : await SleeperAPI.getMatchups(state.leagueId, week);
      ...
      rosterData.players.forEach(function (p) {
        ...
        tr.innerHTML =
          ...
          escapeHtml(p.team) +
          "</td><td>" +
          (p.points !== null ? p.points.toFixed(2) : "-") +
          "</td>";
  ...
  async function renderTransactions() {
    ...
    txns.forEach(function (txn) {
      ...
      var teamsDisplay = detail.teamsWithOwners && detail.teamsWithOwners.length ? detail.teamsWithOwners : detail.teams;
      var headerHtml =
        '<div class="txn-header"><span>' +
        escapeHtml(teamsDisplay.join(" ↔ ")) +
        '</span><span class="txn-type-tag">' +
        escapeHtml(detail.type) +
        "</span></div>";
      var dateHtml =
        '<div class="txn-detail-row">' + new Date(detail.statusUpdated).toLocaleString() + (txn._week ? " — Week " + txn._week : "") + "</div>";
      var addsHtml = detail.adds.length
        ? detail.adds
            .map(function (a) {
              var teamLabel = a.teamWithOwner || a.team;
              return '<div class="txn-detail-row"><span class="add-tag">+ ADD</span> ' + escapeHtml(a.player) + " → " + escapeHtml(teamLabel) + "</div>";
            })
            .join("")
        : "";
      var dropsHtml = detail.drops.length
        ? detail.drops
            .map(function (d) {
              ...
              return (
                '<div class="txn-detail-row"><span class="drop-tag">- DROP</span> ' + escapeHtml(d.player) + " from " + escapeHtml(teamLabel) + "</div>"
              );
            })
            .join("")
        : "";
      var picksHtml = detail.draftPicks.length
        ? detail.draftPicks
            .map(function (dp) {
              return (
                '<div class="txn-detail-row">Draft pick: ' +
                dp.season +
                " Round " +
                dp.round +
                " (" +
      ...
      var faabHtml =
        detail.faab && detail.faab.length
          ? detail.faab
              .map(function (f) {
                return '<div class="txn-detail-row">FAAB: $' + f.amount + " (" + escapeHtml(f.from) + " → " + escapeHtml(f.to) + ")</div>";
              })
              .join("")
          : "";
    ...
    el.textContent = JSON.stringify(
      {
        ...
        scoring_settings_sample: state.league.scoring_settings ? Object.keys(state.league.scoring_settings).slice(0, 5) : []
      },
      null,
      2
    );
  ...
  function openSeasonRecordsTab() {
    ...
    state.seasonRecordsLoadPromise
      ...
      .catch(function (err) {
        ...
        setText("season-records-loading", "Could not load records data. Details: " + (err && err.message ? err.message : err));
      });
  }
  ...
  function renderSeasonRecords() {
    ...
    if (state.seasonRecordsView === "master") {
      records = window.AllTimeStats.buildSeasonMasterRecords(state.allTimeData, state.season, state.seasonRecordsSplit);
    } else {
      var select = byId("season-records-member-select");
      ...
      records = window.AllTimeStats.buildSeasonMemberRecords(state.allTimeData, state.season, state.seasonRecordsSplit, ownerKey);
    }
    grid.innerHTML = "";
  ...
  function renderCareerTotals(allSeasonsData, split) {
    ...
    sorted.forEach(function (owner, idx) {
      ...
      tr.innerHTML =
        ...
        rec.ties +
        "</td><td>" +
        (rec.winPct * 100).toFixed(1) +
        "%</td><td>" +
        rec.pointsFor.toFixed(2) +
        "</td><td>" +
        rec.pointsAgainst.toFixed(2) +
        "</td><td>" +
        champHtml +
        "</td><td>" +
        runnerUpHtml +
  ...
  function renderSummaryCards(containerId, summary, nameA, nameB) {
    ...
    el.innerHTML =
      '<div class="h2h-stat-card"><div class="h2h-stat-value">' +
      summary.totalGames +
      '</div><div class="h2h-stat-label">Games</div></div><div class="h2h-stat-card"><div class="h2h-stat-value">' +
      summary.ownerAWins +
      "-" +
      summary.ownerBWins +
      (summary.ties ? "-" + summary.ties : "") +
      '</div><div class="h2h-stat-label">' +
      escapeHtml(nameA) +
      ' Record</div></div><div class="h2h-stat-card"><div class="h2h-stat-value">' +
      summary.ownerAvgA.toFixed(2) +
      '</div><div class="h2h-stat-label">' +
      escapeHtml(nameA) +
      ' Avg Score</div></div><div class="h2h-stat-card"><div class="h2h-stat-value">' +
      summary.ownerAvgB.toFixed(2) +
      '</div><div class="h2h-stat-label">' +
      escapeHtml(nameB) +
      " Avg Score</div></div>";
  }
  ...
  function renderHeadToHead() {
    ...
    if (result.matchups.length === 0) {
      ...
      if (combinedEl) {
        combinedEl.innerHTML = "<p class=\"status-text\">" + escapeHtml(nameA) + " and " + escapeHtml(nameB) + " have never played each other.</p>";
      }
      tbody.innerHTML = "";
      return;
    }
    ...
    result.matchups.forEach(function (m) {
      ...
      tr.innerHTML =
        ...
        weekLabel +
        "</td><td>" +
        escapeHtml(m.ownerATeamName) +
        "</td><td>" +
        m.ownerAScore.toFixed(2) +
        "</td><td>" +
        m.ownerBScore.toFixed(2) +
        "</td><td>" +
        escapeHtml(m.ownerBTeamName) +
        "</td>";
      tbody.appendChild(tr);
    });
  }
  ...
  function renderOverallCard(containerId, summary) {
    ...
    el.innerHTML =
      ...
      summary.losses +
      (summary.ties ? "-" + summary.ties : "") +
      '</div><div class="h2h-stat-label">Record</div></div><div class="h2h-stat-card"><div class="h2h-stat-value">' +
      (summary.winPct * 100).toFixed(1) +
      '%</div><div class="h2h-stat-label">Win %</div></div><div class="h2h-stat-card"><div class="h2h-stat-value">' +
      summary.avgFor.toFixed(2) +
      '</div><div class="h2h-stat-label">Avg PF</div></div><div class="h2h-stat-card"><div class="h2h-stat-value">' +
      summary.avgAgainst.toFixed(2) +
  ...
  function renderOwnerVsField() {
    ...
    result.byOpponent.forEach(function (row) {
      ...
      tr.innerHTML =
        ...
        reg.ties +
        "</td><td>" +
        reg.pointsFor.toFixed(2) +
        "</td><td>" +
        reg.pointsAgainst.toFixed(2) +
        "</td><td>" +
        po.wins +
        "</td><td>" +
        po.losses +
        ...
        po.ties +
        "</td><td>" +
        po.pointsFor.toFixed(2) +
        "</td><td>" +
        po.pointsAgainst.toFixed(2) +
        "</td>";
      tbody.appendChild(tr);
    });
  }
  ...
  async function init() {
    if (!window.SleeperAPI) {
      showFatalError("sleeper-common.js did not load. Check that assets/sleeper-common.js exists in the repo and that index.html references it correctly.");
      return;
    }
